...
export function publicBase(): string {
  const raw = process.env.SUIPASS_PUBLIC_MCP_BASE ?? `http://localhost:${process.env.PORT ?? 4070}`;
  return raw.replace(/\/+$/, "");
}
...
const dashboardBase = (): string =>
  (process.env.SUIPASS_DASHBOARD_BASE ?? "http://localhost:4071").replace(/\/+$/, "");
...
function redirectUriAllowed(uri: string): boolean {
  ...
  if (url.protocol === "https:") {
    const restrict = (process.env.SUIPASS_OAUTH_REDIRECT_HOSTS ?? "").split(",").map((s) => s.trim()).filter(Boolean);
...
export const acceptedResources = (): string[] => {
  const extra = (process.env.SUIPASS_OAUTH_ACCEPTED_RESOURCES ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
export function oauthRoutes(deps: AppDeps, oauth: OAuthStore): Hono {
  ...
  const accessTtl = () => envInt("SUIPASS_OAUTH_ACCESS_TTL", 3600);
  const refreshTtl = () => envInt("SUIPASS_OAUTH_REFRESH_TTL", 30 * 86_400);

  const registerLimit = new RateLimiter(envInt("SUIPASS_OAUTH_REGISTER_LIMIT", 10), 60_000);
  const authorizeLimit = new RateLimiter(envInt("SUIPASS_OAUTH_AUTHORIZE_LIMIT", 30), 60_000);
  const tokenLimit = new RateLimiter(envInt("SUIPASS_OAUTH_TOKEN_LIMIT", 60), 60_000);
