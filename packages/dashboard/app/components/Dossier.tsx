...
export function Dossier({
  ...
  const connect = heroId
    ? async () => {
        const id = heroId;
        setConnectUrl(`${window.location.origin}/c/${id}/mcp`);
        setConnectOpen(true);
      }
    : undefined;
  const rotate = heroId
        ...
        {card ? (
          <>
            <div className="dbody-wrap" data-tour="readings">
              <AnimatePresence initial={false}>
                <HeadBody key={card.id} card={card} />
              </AnimatePresence>
            </div>
...
function Carousel({
  ...
  kmap: Map<string, string>;
  kAgent?: string;
  fiatMap?: Map<string, never>;
  onSelect?: (cardId: string) => void;
  onIssue?: () => void;
}) {
  const idx = Math.max(0, roots.findIndex((n) => n.card.id === currentId));
  ...
  const goTo = (i: number) => {
    ...
    if (roots[i].card.id !== currentId) onSelect(roots[i].card.id);
  };
          ...
          return (
            ...
            >
              <CardHero
                card={n.card}
                holder={n.card.name}
                agentAddress={kmap.get(n.card.id) ?? (active ? kAgent : undefined)}
  ...
  return (
    <div className="verbcol" data-tour="verbs">
      <div className="verbs">
          ...
          <>
            ...
            {card.status === "active" && (
              <button
                className="vpill"
                disabled={busy}
                onClick={act(() => api.freeze(card.id), "Freeze")}
                data-testid="freeze"
            ...
            {frozen && (
              <button
                ...
                disabled={busy}
                onClick={act(() => api.unfreeze(card.id), "Unfreeze")}
        ...
        {dead && <DeleteButton cardId={card.id} hasKids={hasKids} onDone={onDeleted ?? refresh} />}
      </div>
