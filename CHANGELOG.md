...
- Expanded the in-app `/docs` reference to full parity with the README and code. Surfaced the `card` tool's on-chain `account` (the root delegator) so agents route swap recipients to it. Rounded out Contract Cards: `perTradeMax` is enforced on the USDC / settlement leg only in v1, added the `token_not_allowed` and `spender_not_allowed` refusals, and noted exact-spender pinning. Expanded the optional-environment table (`REMIT_RPC_URL`, `REMIT_DB_PATH`, `REMIT_ALLOWED_HOSTS`, `BASESCAN_API_KEY`, `REMIT_TRUST_PROXY_HOPS`, the OAuth TTL / redirect / rate-limit vars). Added wallet private-key export to the Security section. Mirrored the README's per-track "Code usage" deep links into the Cook Off section, with a note that ERC-7715 Advanced Permissions is intentionally not used. Extended the connection-lane one-liners (hermes, the OpenClaw SSE caveat, the OAuth out-of-band paste-back, Gemini CLI, Windsurf, ChatGPT, claude.ai) and named the 1Shot Public Relayer in the x402 prose. Removed all emdashes from the page.
